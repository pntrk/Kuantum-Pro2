...
code = code.replace(/<h2 className="text-xl font-bold text-slate-800 mb-6 border-b pb-2">Ders Saatleri \(Zil\)<\/h2>\s*\}\)\}\s*<\/div>/, '<h2 className="text-xl font-bold text-slate-800 mb-6 border-b pb-2">Ders Saatleri (Zil)</h2>\n                  ' + replacement);

fs.writeFileSync('src/App.tsx', code);
console.log("Restored Zil settings");
