...
const calcFunction = `  // Safe Set-based calculation for total workload (Table + Pool) to prevent duplicate counting
  const calculateSafeTotalWorkload = (entityType, entityName) => {
      ...
      // 2. Check Placed Courses (Timetable)
...
// Insert after unplacedCourses definition or parseCellData
const insertRegex = /(const parseCellData = \(cellVal\) => \{[\s\S]*?return null;\s*\};)/;
content = content.replace(insertRegex, `$1\n\n${calcFunction}`);

// Replace the line in renderConstraintModal
const replaceRegex = /\{totalPlacedHours \+ relatedCardsUnplaced\.reduce\(\(acc, curr\) => acc \+ parseInt\(curr\.hours \|\| 0, 10\), 0\)\} <span className="text-xs">Saat<\/span>/;
const replacementCall = `{calculateSafeTotalWorkload(type, name)} <span className="text-xs">Saat</span>`;
content = content.replace(replaceRegex, replacementCall);

fs.writeFileSync('src/App.tsx', content);
console.log("Success replacing workload calculation");
